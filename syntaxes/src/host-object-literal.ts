...
export const HostObjectLiteral: GrammarDefinition = {
  ...
  repository: {
    hostObjectLiteral: {
      // Match `host: {`. Note that we include the opening brace, because we don't want
      // capture something like `host: SOME_CONST`.
      begin: /(host)\s*(:)\s*{/,
      beginCaptures: {
      ...
      patterns: [
        // Value can be either dynamic or static and we have different patterns for them.
    ...
    ngHostBindingStatic: {
      ...
      beginCaptures: {
        ...
        // Colon is shown as JS syntax.
        4: {name: 'meta.object-literal.key.ts punctuation.separator.key-value.ts'},
        // Opening quote for the value is shown as a string. Allows backticks as well. The only
        // reason this is in a separate group is so we can reference it for the closing quote.
        5: {name: 'string'},
        // Remainder of the value is shown as a string.
        6: {name: 'string'},
      },
